...
6. Assemble the runtime sheet: `node scripts/sprites/generate-character-mcp.mjs --id=<sprite-id> --zip=<path>`. It identifies animations by frame count, requires eight directions, centres each frame in the 92px cell (cropping larger padded exports, padding the exact-size frames pro mode now returns), and writes the manifest-implied 736x920 `sheet.png`.
