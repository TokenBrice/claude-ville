...
1. **Code Forge — Edit/Write/MultiEdit/apply_patch:** anvil hammer swing accelerates and emits a one-shot spark ring with each new tool call; idle forge glow drops to baseline within ~2 s of last edit. (Today the hammer arc at `BuildingSprite.js:1620-1658` runs continuously off `this.frame * 0.18` regardless of activity.)
2. **Lore Archive — Read/Grep/Glob/LS:** a tome icon at the archive doorway flips one page per Read/Grep call; the page-flip lasts ~600 ms and the basename of `file_path` (or `pattern` for Grep) materializes briefly above the building. Falls back to a single mote burst when the tool input is missing.
...
4. **Clock Observatory — WebSearch/WebFetch/`web.run`:** a small telescope barrel rotates from rest to point at a host-derived angle; a star-arc trails behind for ~900 ms; the URL host displays as a 6-char compact label below the dome. The existing clock face (`OBSERVATORY_CLOCK_FACE`, `BuildingSprite.js:112-121`) is untouched.
5. **Portal Gate — `mcp__playwright__*`, `functions.spawn_agent`/`send_input`/`wait_agent`:** rune-ring opacity boost and a ghost-mirror panel above the gate displays the tool's hostname or URL. Augments — not replaces — the existing visitor-driven boost at `BuildingSprite.js:976-991`.
6. **Harbor Master — git tool calls (extends existing):** when an agent at the harbor visit-tile is mid-`git diff`/`git status`, a single repo-themed crate appears on the quay; on `git push` the crate transitions onto the existing ship in `HarborTraffic.js`. Uses pre-existing `gitEvents` flow; does not invent a new event source.
...
9. **Pharos Lighthouse — passive activity proxy:** beam rotation rate scales linearly with concurrent active-WORKING agent count (1×–2.4×); beam tints from gold to orange-red when any agent has `pushStatus === 'failed'` in the harbor traffic state. Uses T0.3 `LightSourceRegistry` beam entry already declared at `BuildingSprite.js:92-105`.
...
- **TR0.1 — Tool-event observer.** A new module under `src/presentation/character-mode/` (or `src/application/` if other consumers emerge) that watches `agent:updated` events and emits `tool:invoked { agentId, tool, input, ts, building }` whenever it sees a transition where `(agent.id, lastTool, lastToolInput, lastSessionActivity)` differs from the prior snapshot. Effort: S. Lives in presentation today (the only consumer is the renderer); promote to application later if dashboard mode needs it. Rationale: this is the single load-bearing design call. Two options:
...
**1.1 Implement TR0.1 tool-event observer**
**Domain:** Application/Presentation · **File:** new `src/presentation/character-mode/ToolEventObserver.js` (or `src/application/ToolEventObserver.js` if dashboard mode adopts later) · **Effort:** S · **Impact:** Medium (gate for everything else) · **Depends on:** none.

Subscribe to `agent:updated` via `eventBus`. Maintain `Map<agentId, { tool, input, sessionActivity }>`. On change, emit a new domain event `tool:invoked { agentId, tool, input, building, ts }` where `building = agent.targetBuildingType`. Reset snapshot on `agent:removed`. No-op when `agent.status !== AgentStatus.WORKING`. Reduced-motion: still emit (rituals decide whether to render); no motion-budget check at the observer layer.
