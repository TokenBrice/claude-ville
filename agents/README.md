...
| [`plans/claudeville-fable-5.1-enhancement-implementation-plan.md`](plans/claudeville-fable-5.1-enhancement-implementation-plan.md) | `proposed — not started` | Fable 5.1 enhancement plan (*The Commander's Map*): five cross-item contracts and 25 items in four waves against `v0.37.0` — truth hotfixes (pricing, identity, cold scans, overlays), operator signal (provider-reported cost, turn timing, working set, hook ingestion), GPU-path light delivery (ladder, parity, attention lights, labels), and the chrome as an instrument. |
