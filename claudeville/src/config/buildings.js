export const BUILDING_DEFS = [
    { type: 'command', x: 18, y: 18, width: 5, height: 4, label: 'COMMAND CENTER', icon: 'C', description: 'Team status' },
    { type: 'forge', x: 28, y: 15, width: 4, height: 3, label: 'CODE FORGE', icon: 'F', description: 'Code work' },
    { type: 'mine', x: 12, y: 24, width: 4, height: 3, label: 'TOKEN MINE', icon: 'T', description: 'Token usage' },
    { type: 'taskboard', x: 25, y: 25, width: 3, height: 3, label: 'TASK BOARD', icon: '#', description: 'Task status' },
    { type: 'chathall', x: 15, y: 14, width: 4, height: 3, label: 'CHAT HALL', icon: 'M', description: 'Messages' },
    { type: 'observatory', x: 9, y: 13, width: 4, height: 4, label: 'RESEARCH OBSERVATORY', icon: '*', description: 'External research' },
    { type: 'archive', x: 6, y: 21, width: 4, height: 3, label: 'LORE ARCHIVE', icon: 'A', description: 'Reading and search' },
    { type: 'portal', x: 31, y: 23, width: 4, height: 4, label: 'PORTAL GATE', icon: 'O', description: 'Browser and remote tools' },
    { type: 'alchemy', x: 23, y: 9, width: 3, height: 3, label: 'PROMPT ALCHEMY', icon: 'P', description: 'Notebook and prompt work' },
    { type: 'sanctuary', x: 9, y: 7, width: 4, height: 3, label: 'IDLE SANCTUARY', icon: 'S', description: 'Resting agents' },
    { type: 'watchtower', x: 34, y: 10, width: 3, height: 5, label: 'HARBOR LIGHTHOUSE', icon: 'L', description: 'Harbor monitoring' },
];
